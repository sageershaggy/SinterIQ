...
**Mailbox** now provides Inbox, Outbox, Sent and private drafts, searchable by company, subject or email. Optional encrypted IMAP configuration enables read-only incoming sync. Matching app-generated message references and sender addresses links replies to companies; unmatched messages can be linked explicitly. Linked conversations support deliberate replies with threading headers. Incoming replies append history and stop applicable follow-ups, preserving qualification and terminal outcomes. Each project has its own mailbox, cursor and history: a reply can only be matched to a lead in the project whose mailbox received it, and two projects pointed at one inbox each collect their own copy. The worker polls every enabled mailbox in turn and pauses automatic deliveries for a project whose own incoming connection is failing, leaving the other projects running.
