...
Activation requires that project's own configured mailbox, a copy address and a public HTTPS app origin. The worker reserves one due enrollment per minute per project in SQLite, skips any project whose enabled inbox is failing, checks the enrolling account's current access and the lead's qualified revision, and repeats those checks before SMTP connection. DNS addresses are validated and pinned, with TLS certificate checks against the original hostname. Plain-text and HTML messages include identity and opt-out information. Copy delivery uses BCC; partial recipient acceptance is reported as uncertain rather than successful.
