...
test('every Filters facet narrows the lead list on the server', async () => {
  const f = fixture();
  try {
    const { names, adminId, list, ids, project } = await seeded(f);
    const all = [
      'Alpha Pumps',
      'Beta Law',
      'Delta Trading',
    ...
    // Email status is the outreach status the mail and funnel code keeps.
    assert.deepEqual(await names('email_status=REPLIED'), ['Beta Law']);
    assert.deepEqual(await names('email_status=CONTACTED&email_status=REPLIED'), [
      'Alpha Pumps',
      'Beta Law',
    ]);

    // Lead status is the manual CRM status: New until someone moves it, then the latest move.
    const move = f.db.prepare(
      'INSERT INTO lead_status_events (project_id,lead_id,from_status,to_status,created_by_id,created_by,created_at) VALUES (?,?,?,?,?,?,?)',
    );
    const at = new Date().toISOString();
    move.run(project.id, ids['Beta Law'], 'NEW', 'CONTACTED', adminId, 'Test Administrator', at);
    move.run(project.id, ids['Beta Law'], 'CONTACTED', 'INTERESTED', adminId, 'Test Administrator', at);
    move.run(project.id, ids['Gamma Legal'], 'NEW', 'CONTACTED', adminId, 'Test Administrator', at);
    assert.deepEqual(await names('lead_status=INTERESTED'), ['Beta Law']);
    assert.deepEqual(await names('lead_status=CONTACTED'), ['Gamma Legal']);
    assert.deepEqual(
      await names('lead_status=NEW'),
      all.filter((name) => name !== 'Beta Law' && name !== 'Gamma Legal'),
    );
...
test('the CSV export selects exactly the rows the list shows, in the same order', async () => {
  ...
  try {
    ...
    const queries = [
      ...
      'assignee=none&email_status=NOT_CONTACTED&lead_status=NEW&sort=industry_asc',
      'assignee=' + adminId,
