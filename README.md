...
1. Every project has its own mailbox; there is no workspace mailbox. An administrator opens the project's **Mailbox**, chooses **Configure email** and sets the outgoing SMTP submission host, username, password/app password, sender name and address, optional Reply-to and **Copy every outreach email to**. Copies use BCC. Passwords remain encrypted on the server and are never returned to the browser. **Mailbox** appears in a project's navigation for administrators only; **Send a test** delivers to that project's own sender address, proving the setup before any lead is mailed. A project without a configured mailbox cannot send at all.
...
4. Review the sequence and explicitly start it. Starting requires `INNOVISTA_ORIGIN` set to the public HTTPS app origin, plus that project's own configured mailbox and copy address. Keep the app server running and that origin reachable for unsubscribe links. Local drafts can be prepared without delivery setup.
5. The server processes at most one due message per minute per project, persists progress across restarts, and supports pause/resume. Each project paces its own queue, so a busy project never delays another's next message. There is no catch-up burst. Pausing cannot recall messages already handed to SMTP.
6. Open the project's **Mailbox** for its Inbox, Outbox, Sent and your own drafts; each folder shows only that project's mail. Incoming IMAP is optional and lives under the same **Configure email**; click **Verify & sync** to verify the saved connection without sending an email. Matched replies appear on their lead and stop applicable follow-ups; unmatched mail is linked to a company in the same project before replying, never to a lead in another project. Record interest, conversion and unsubscribe requests deliberately in the lead's **Email** tab. Qualification remains unchanged. Unsubscribe links suppress further emails automatically after confirmation or an email client's one-click POST.

Every lead's **Email** button opens its saved draft or the **Support email** template. Edit it, choose another project/starter template or use the block designer, then click **Send**. Connecting email never sends saved drafts automatically. See [mailbox setup](docs/mailbox-setup.md) for incoming authentication, sync limits and provider requirements; its connection steps still describe the former workspace screen, which now lives in each project.

A reply is matched only against mail the same project sent, so a project's mailbox can never surface a reply on another project's lead. Two projects may point at one inbox, but each polls it separately and ingests its own copy of every message: both teams then see the same reply, and the incoming settings name the other projects already polling that inbox so the choice is deliberate. Prefer one address per project. If a project's IMAP connection fails, that mailbox records the error and is skipped; polling and scheduled delivery continue for every other project.

Upgrading an existing installation copies the previous workspace mailbox onto every project that already exists, so sending keeps working unchanged. If incoming mail was enabled, polling stays on for only the oldest project, which also inherits the IMAP resume point and the messages already received; the other projects get the same settings, disabled, for an administrator to confirm or repoint to their own address. The old workspace settings stay in the database untouched and unread. Review each project's mailbox after upgrading, because every project now sends under its own sender address.

Individual outreach and funnels share a maximum of three accepted or uncertain sends per recipient across the workspace, no matter which project's mailbox sends them. Opt-outs and send counts survive lead deletion and reimport. A changed lead, training version or project assignment blocks scheduled delivery pending review. Failed or interrupted SMTP attempts are never automatically retried when delivery may already have happened; inspect the mailbox before contacting again.
