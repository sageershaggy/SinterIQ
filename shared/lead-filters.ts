...
export const callStatusHints: Record<CallStatus, string> = {
  ...
  FOLLOW_UP: 'The last call asked for another one',
  NONE: 'Not assigned and never called',
};
/**
 * Which call status the most recent logged outcome puts a lead in. Adding a call outcome means
 * adding it here too; the type makes that a compile error rather than a silent miss.
 */
export const callOutcomeStatus: Record<CallOutcome, Exclude<CallStatus, 'ASSIGNED' | 'NONE'>> = {
  CONNECTED: 'COMPLETED',
  MEETING_BOOKED: 'COMPLETED',
  NOT_INTERESTED: 'COMPLETED',
  NO_ANSWER: 'PENDING',
  CALLBACK: 'FOLLOW_UP',
  WRONG_CONTACT: 'FOLLOW_UP',
};

/**
 * Lead status. Today this is the outreach status the mail and funnel code maintains; when a
 * CRM status column arrives, add its values here and point the server's leadStatusSql at it.
 */
