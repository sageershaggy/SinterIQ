 ...
 *
 * lead_research_runs: one row per website research pass, including the passes that found
...
export function installActivitySchema(db: DB) {
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS research_runs_project ON lead_research_runs(project_id, id DESC);
    CREATE INDEX IF NOT EXISTS research_runs_lead
