...
export default function Mailbox({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      api<MailPage>(`${base}?folder=${folder}&page=${page}&q=${encodeURIComponent(query)}`)
        .then((result) => {
          if (!cancelled) {
            setData(result);
            setError('');
  ...
  }, [base, folder, page, query, refresh]);
  useEffect(() => {
  ...
  useEffect(() => {
    // A project with no mailbox yet has nothing to read, so the first screen is the form
    // that gives it one. Only offered once, so hiding it stays hidden.
    if (!data || offered.current) return;
    offered.current = true;
    if (!data.outgoing_configured && !data.incoming.enabled) setConfiguring(true);
  }, [data]);
  async function open(row: MailRow) {
    ...
    if (row.kind === 'incoming' && row.status === 'UNREAD') {
      try {
        await api(base + '/incoming/' + row.id + '/read', { method: 'POST' });
        setSelected((current) =>
  ...
  return (
    <>
      <div className="page-heading">
        <div>
          <span className="eyebrow">THIS PROJECT’S CONVERSATIONS</span>
          <h1>
            Mailbox<span className="heading-dot">.</span>
          </h1>
          <p>
            Incoming messages, scheduled follow-ups and sent emails for {project.name}, connected to
            each company.
          </p>
        </div>
        <div className="mail-heading-actions">
          ...
          >
            {configuring ? <Inbox size={15} /> : <Settings size={15} />}
            {configuring ? 'Back to messages' : 'Configure email'}
          </button>
          <button
            ...
            onClick={async () => {
              ...
              setError('');
              try {
                const result = await api<{ received: number }>(base + '/sync', { method: 'POST' });
        ...
        <section className="mail-center">
          ...
          <div className="mail-content">
            ...
            <div className={'mail-split ' + (selected ? 'has-message' : '')}>
              <div
                className="mail-list"
                aria-label={label(folder) + ' messages'}
                aria-busy={loading}
              >
                {loading && !data ? (
                    ...
                    title={
                      query
                        ...
                        : folder === 'inbox' && !data?.incoming.enabled
                          ? 'Connect this project’s inbox'
                          : 'No messages here yet'
                    }
                    action={
                      !query && folder === 'inbox' && !data?.incoming.enabled ? (
                        <button className="button secondary" onClick={() => setConfiguring(true)}>
                  ...
                  >
                    {folder === 'inbox'
                      ? `Connect the inbox for ${project.name} in its mailbox settings, then sync it. Only mail this project receives appears here.`
                      : `Emails and campaigns for leads in ${project.name} will appear here.`}
                  </Empty>
                )}
              ...
              <article className="mail-reader" aria-label="Message details">
                {!selected ? (
                  <div className="mail-reader-empty">
                    <Mail size={32} />
                    <h3>Every {project.name} conversation, in one place</h3>
...
function LinkMessage({
  ...
  useEffect(() => {
    let cancelled = false;
    // A narrower search can drop the chosen company, so the selection resets with the list.
    setLeadId('');
    const timer = setTimeout(() => {
      api<{ leads: Lead[] }>(
        `/projects/${project.id}/leads?search=${encodeURIComponent(query)}&page_size=50`,
      )
        .then((result) => {
  ...
  return (
    <form
      ...
      onSubmit={async (e) => {
        ...
        setError('');
        try {
          await api(`/projects/${project.id}/mailbox/incoming/${message.id}/link`, {
            method: 'POST',
            body: json({ lead_id: Number(leadId) }),
          });
          onLinked();
        } catch (e) {
          setError((e as Error).message);
    ...
    >
      ...
      <p className="muted">
        Choose the company in {project.name} that it belongs to, so the project team can see it and
        reply. Linking records a response and stops applicable scheduled follow-ups.
      </p>
      {error && <Alert>{error}</Alert>}
      <label>
        Find company
