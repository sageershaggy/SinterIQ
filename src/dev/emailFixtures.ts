 ...
 * Lead 9 is a qualified, high-scoring company, so the composer pre-selects the high-quality
...
export function emailRoutes(lead: { name: string; score: number | null; contact_email: string }) {
  return [
    ...
    [/^\/projects\/2\/leads\/9\/email\/draft$/, () => offer(qualifiedLead, true)],
    [/^\/projects\/2\/leads\/\d+\/incoming/, () => []],
    [
      /^\/projects\/2\/leads\/9\/email\/files$/,
      () => ({
